...
class ImagePerturbationPipeline:
    ...
    def get_pollution_type(self, method: str) -> str:
        """
    ...
    def discover_images(self, images_dir: Path) -> List[Tuple[int, Path]]:
        """Discover all image files in the images directory."""
        images = []

        for profile_id in os.listdir(images_dir):
            for image_file in os.listdir(os.path.join(images_dir, profile_id)):
                if image_file.endswith(".png"):
                    images.append((profile_id, os.path.join(images_dir, profile_id, image_file)))
    ...
    def run(
        ...
        synthetic_images_dir = os.path.join(self.args.output_dir, "images")
        output_path = os.path.join(self.args.output_dir, "images_perturbed")
        
        images = self.discover_images(synthetic_images_dir)
        if not images:
            logger.error(f"No image files found in {synthetic_images_dir}")

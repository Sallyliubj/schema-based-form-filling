...
class ImageGenerationPipeline:
    ...
    def get_available_forms(self, directory: str) -> List[str]:
        """Get list of forms that have generated values."""
        available = []
        for form_type in self.args.value_filling_forms + self.args.llm_forms:
            values_path = os.path.join(directory, f"{form_type}.json")
            if os.path.exists(values_path):
                available.append(form_type)
        return available


    def generate_form_image(self, form_type: str, profile_id: int) -> Optional[str]:
        """
        ...
        values_path = os.path.join(self.args.output_dir, "values", str(profile_id), f"{form_type}.json")
        ...
        os.makedirs(os.path.join(self.args.output_dir, "images", str(profile_id)), exist_ok=True)
        output_path = os.path.join(self.args.output_dir, "images", str(profile_id), f"{form_type}_synthetic.png")
    ...
    def run(self) -> Dict[str, Optional[str]]:
        ...
        # for each profiles in the values directory, generate the form images
        for profile_id in os.listdir(os.path.join(self.args.output_dir, "values")):

            available_forms = self.get_available_forms(os.path.join(self.args.output_dir, "values", str(profile_id)))

            if not available_forms:
                print(f"\n✗ No form values found for profile {profile_id}")
                continue
            ...
            for form_type in available_forms:
                ...
                self.generate_form_image(form_type, profile_id)
